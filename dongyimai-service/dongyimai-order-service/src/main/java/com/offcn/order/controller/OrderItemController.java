...
public class OrderItemController {
    ...
    public Result<PageResult<OrderItem>> findPage(@RequestBody(required = false) @ApiParam(name = "OrderItem对象",value = "传入JSON数据",required = false) OrderItem orderItem, @PathVariable  int page, @PathVariable  int size){
        //调用OrderItemService实现分页条件查询OrderItem
        PageResult<OrderItem> pageResult = orderItemService.findPage(orderItem, page, size);
    ...
    public Result<PageResult<OrderItem>> findPage(@PathVariable  int page, @PathVariable  int size){
        //调用OrderItemService实现分页查询OrderItem
        PageResult<OrderItem> pageResult = orderItemService.findPage(page, size);
        return new Result<PageResult<OrderItem>>(true, StatusCode.OK,"查询成功",pageResult);
    }
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "OrderItem对象",value = "传入JSON数据",required = false) OrderItem orderItem, @PathVariable Long id){
        //设置主键值
        orderItem.setId(id);
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @GetMapping("/{id}")
    public Result<OrderItem> findById(@PathVariable Long id){
